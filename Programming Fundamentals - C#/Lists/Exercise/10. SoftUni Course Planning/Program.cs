    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((command = Console.ReadLine()) != "course start")
            {
                ...
                {
                    case "Add":
                        string lessonTitleToAdd = cmdArgs[1];
                        if (!courseSchedule.Contains(lessonTitleToAdd))
                        {
                            courseSchedule.Add(lessonTitleToAdd);
                        }
                        break;

                    case "Insert":
                        string lessonTitleToInsert = cmdArgs[1];
                        int indexToInsertAt = int.Parse(cmdArgs[2]);
                        if (!courseSchedule.Contains(lessonTitleToInsert))
                        {
                            courseSchedule.Insert(indexToInsertAt, lessonTitleToInsert);
                        }
                        break;

                    case "Remove":
                        string lessonTitleToRemove = cmdArgs[1];
                        string exerciseTitleToRemove = lessonTitleToRemove + "-Exercise";
                        if (courseSchedule.Contains(lessonTitleToRemove))
                        {
                            courseSchedule.Remove(lessonTitleToRemove);
                            if (courseSchedule.Contains(exerciseTitleToRemove))
                            {
                                courseSchedule.Remove(exerciseTitleToRemove);
                            }
                        }
                        break;
